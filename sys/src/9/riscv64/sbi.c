 ...
 *	upon resumption, hart will see: R10 is hartid, R11 is resume_new_a1,
 *		satp 0, sie 0.
 * 1	hart_stop(void);
 * 2	hart_status(ulong hartid);
 *
 * extensions present in OpenSBI v0.6 (allegedly SBI v0.2):
 * 0-8, 0x10, 0x735049 (sPI), 0x52464e43 (RFNC)
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
vlong
sbigetmvendorid(void)
{
	Sbiret sbiret;

	if (sbicall(0x10, 4, 0, &sbiret, nil) < 0)
		return -1;
	return sbiret.status;
}
...
sbigetmarchid(void)
{
	...
	if (sbicall(0x10, 5, 0, &sbiret, nil) < 0)
		return -1;
	return sbiret.status;
}
...
{
	/* this is the legacy call, not the newer sPI extension */
...
sbihartsuspend(void)
{
	...
	xargs[0] = 0;			/* for r11 */
	xargs[1] = 0;			/* for r12 */
	return sbicall(HSM, HSMSUSP, 0, nil, xargs);
}

void
sbireset(ulong type, ulong reason)
{
	uvlong xargs[2];

	xargs[0] = reason;		/* for r11 */
	xargs[1] = 0;			/* for r12 */
	sbicall(SRST, 0, type, nil, xargs);
}
