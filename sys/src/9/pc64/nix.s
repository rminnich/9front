#include "mem.h"
//#include "io.h"
#define VectorDE 1 /* debug exception */
#define VectorNMI 2 /* non-maskable interrupt */
#define VectorBPT 3 /* breakpoint */
#define VectorUD 6 /* invalid opcode exception */
#define VectorCNA 7 /* coprocessor not available */
#define Vector2F 8 /* double fault */
#define VectorCSO 9 /* coprocessor segment overrun */
#define VectorSNP 11 /* segment not present */
#define VectorGPF 13 /* general protection fault */
#define VectorPF 14 /* page fault */
#define Vector15 15 /* reserved */
#define VectorCERR 16 /* coprocessor error */
#define VectorAC 17 /* alignment check */
#define VectorMC 18 /* machine check */
#define VectorSIMD 19 /* simd error */

#define VectorPIC 32 /* external i8259 interrupts */
#define IrqCLOCK 0
#define IrqKBD 1
#define IrqUART1 3
#define IrqUART0 4
...
#define VectorLAPIC VectorPIC+16 /* local APIC interrupts */
#define IrqLINT0 16 /* LINT[01] must be offsets 0 and 1 */
#define IrqLINT1 17
#define IrqTIMER 18
#define IrqERROR 19
#define IrqPCINT 20
#define IrqSPURIOUS 31 /* must have bits [3-0]  0x0F */
#define MaxIrqLAPIC 31

#define VectorIPI 62
#define VectorSYSCALL 64

#define VectorAPIC 65 /* external APIC interrupts */
#define MaxVectorAPIC 255// NIX stuff, just keep it here.
#define M_PROC 16
...
CALL _acintrp<>(SB); BYTE $0 /* #DE Divide-by-Zero Error */
CALL _acintrp<>(SB); BYTE $VectorDE /* #DB Debug */
CALL _acintrp<>(SB); BYTE $VectorNMI /* #NMI Borked */
CALL _acintrp<>(SB); BYTE $VectorBPT /* #BP Breakpoint */
CALL _acintrp<>(SB); BYTE $/*VectorOF*/4 /* #OF Overflow */
CALL _acintrp<>(SB); BYTE $/*VectorBR*/5 /* #BR Bound-Range */
CALL _acintrp<>(SB); BYTE $VectorUD /* #UD Invalid-Opcode */
CALL _acintrp<>(SB); BYTE $VectorCNA /* #NM Device-Not-Available */
CALL _acintre<>(SB); BYTE $Vector2F /* #DF Double-Fault */
CALL _acintrp<>(SB); BYTE $VectorCSO /* reserved */
CALL _acintre<>(SB); BYTE $10 /* #TS Invalid-TSS */
CALL _acintre<>(SB); BYTE $VectorSNP /* #NP Segment-Not-Present */
CALL _acintre<>(SB); BYTE $12 /* #SS Stack */
CALL _acintre<>(SB); BYTE $VectorGPF /* #GP General-Protection */
CALL _acintre<>(SB); BYTE $VectorPF /* #PF Page-Fault */
CALL _acintrp<>(SB); BYTE $15 /* reserved */
CALL _acintrp<>(SB); BYTE $VectorCERR /* #MF x87 FPE-Pending */
CALL _acintre<>(SB); BYTE $VectorAC /* #AC Alignment-Check */
CALL _acintrp<>(SB); BYTE $VectorMC /* #MC Machine-Check */
CALL _acintrp<>(SB); BYTE $VectorSIMD /* #XF SIMD Floating-Point */
...
CALL _acintrp<>(SB); BYTE $VectorPIC
